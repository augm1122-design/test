...
<head>
 ...
 <style>
  ...
  /* خصائص الصورة */
   ...
   /* تُحدد تنسيق الرسائل التي تظهر للمستخدم نجاح أو خطأ */
   .message { 
       text-align: center;
       margin-bottom: 15px;
       padding: 10px;
  ...
  /* يُعيد ترتيب العناصر إلى عمود واحد عندما تكون الشاشة صغيرة */
...
<body>
 ...
 <h2>Sign In</h2>
 <!-- المكان الذي ستظهر فيه رسائل النجاح أو الخطأ من الخادم  -->
 <div id="responseMessage" class="message"></div> <form id="signInForm"> <b>Email</b><!-- بداية نموذج تسجيل الدخول -->
 <!-- لادخال الايميل -->
   ...
   <button class="btn btn-primary" type="submit">Sign In</button>
    <!-- زر المتابعة كزائر -->
   <a href="dorm_type.php" class="btn-guest-link">Continue as Guest</a> </form>
   <!-- رابط انشاء حساب جديد -->
 <div class="register-link">
 Don’t have an account? <a href="create_account.php">Click here to create one.</a>
 </div>
 </div>
 <!-- عمود الصورة -->
 ...
 <script>
  // التعامل مع إرسال النموذج
   document.getElementById('signInForm').addEventListener('submit', function(event) {
    ...
    // يخزن قيم حقول الادخال في متغيرات 
     ...
     responseMessageDiv.className = 'message'; // إعادة تعيين الفئات
    // التحقق من أن كلا الحقلين غير فارغين
     if (email && password) {
       fetch('signin_process.php', {// إرسال البيانات إلى signin_process.php باستخدام Fetch API
         method: 'POST',
         headers: {
           'Content-Type': 'application/x-www-form-urlencoded',
         },
         body: new URLSearchParams({ // إرسال البيانات كـ URL-encoded
           email: email,
           password: password,
         }).toString()
       })
       .then(response => {
           if (!response.ok) {//تحقق أولاً من أن الاستجابة ناجحة 
               return response.text().then(text => { throw new Error(text); });
           }
           return response.json(); // حاول تحليل الاستجابة كـ JSON
       })
       .then(data => {// التعامل مع البيانات المستلمة من الخادم
         responseMessageDiv.style.display = 'block';
         responseMessageDiv.textContent = data.message;//بتبين رسالة النجاح او الخطأ
         responseMessageDiv.className = 'message ' + data.status; // تطبيق فئة النجاح/الخطأ
        
         //ذا كان تسجيل الدخول ناجحاً، يُعيد توجيه المستخدم إلى صفحة أخرى
         if (data.status === 'success') {
           window.location.href = data.redirect; 
         }
       })
       //يلتقط أي أخطاء تحدث أثناء عملية fetch
       .catch(error => {
         console.error('Fetch Error:', error); // عرض الخطأ الكامل في الكونسول
         responseMessageDiv.style.display = 'block';
         responseMessageDiv.textContent = 'حدث خطأ أثناء الاتصال بالخادم. الرجاء المحاولة لاحقاً.';
         responseMessageDiv.className = 'message error';
       });
       //إذا كانت حقول الإدخال فارغة، يظهر رسالة خطأ مباشرة دون إرسال طلب إلى الخادم
     } else {
       responseMessageDiv.style.display = 'block';
       responseMessageDiv.textContent = "الرجاء تعبئة كلا الحقلين (البريد الإلكتروني وكلمة المرور).";
       responseMessageDiv.className = 'message error';
     }
   });
 </script>
